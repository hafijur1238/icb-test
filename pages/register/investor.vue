...
const { data, status, error, refresh, clear } = await useFetch(
  "/v1/api/location/division"
);
// console.log(JSON.parse(JSON.stringify(data)));
// console.log(JSON.parse(JSON.stringify(data?.value))?.body);
...
const form = reactive({
  ...
  passportImage: "",
  image: "",
  division: JSON.parse(JSON.stringify(data?.value))?.body,
  //  division: "",
  district: "",
  thana: "",
  address: "",
      ...
      >
        <div>
          ...
          <select
            ...
            class="input-field w-full"
          >
            <option value="Select Division" disabled selected>
              Select Division
            </option>
            <option
              v-for="division in form.division"
              :key="division"
              :value="division"
            >
              {{ division?.nameBn }}
